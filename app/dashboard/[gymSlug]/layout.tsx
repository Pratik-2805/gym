...
export default async function DashboardLayout(props: DashboardLayoutProps) {
  ...
  const hasAccess = isSuper || session.gymId === gym.id;

  if (!hasAccess) {
    redirect('/login');
  }

  // Active user name details
  const activeUser = gym.users.find((u: any) => u.id === session.userId) || { name: 'Gym Owner', email: session.email };
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="overflow-hidden">
              ...
              <span className="flex items-center gap-1 text-[9px] font-semibold text-zinc-500 mt-0.5 uppercase tracking-wider">
                <UserCheck className="h-2.5 w-2.5 text-cyan-400" /> {session.role}
              </span>
            </div>
          </div>
